...
const atrsoByEstudiante = (req, res, next) => {
  ...
  atrasoService
    .atrasoByEstudiante(estudianteId)
    .then((esquela) => {
      if (!esquela) {
        res.status(404).json({ message: "Esquelas no encontradas" });
      } else {
...
const generateReportFile = async (res, fileName, reporte) => {
  ...
  for (let i = 0; i < reporte.length; i++) {
    ...
    ws.cell(cualFila, 1)
      .string(reporteActual.estudiante.cedula)
      .style(contenidoEstilo);
    ws.cell(cualFila, 2)
      .string(reporteActual.registroFecha)
      .style(contenidoEstilo);
    ws.cell(cualFila, 3)
      .string(reporteActual.registroHora)
      .style(contenidoEstilo);
    ws.cell(cualFila, 4)
      .string(reporteActual.estudiante.NombreEst)
      .style(contenidoEstilo);
    ws.cell(cualFila, 5)
      .string(reporteActual.estudiante.ApellidoEst)
      .style(contenidoEstilo);
    ws.cell(cualFila, 6)
      .string(reporteActual.estudiante.curso_id)
      .style(contenidoEstilo);
    ws.cell(cualFila, 7)
      .string(reporteActual.descripcion)
      .style(contenidoEstilo);
...
module.exports = {
  ...
  findByInspector,
  atrsoByEstudiante,
  generarReportesByCurso,
  generarReportesByEstudiante,
  generarReportesByFecha,
};
